...
const SuggestInputSchema = SuggestMedicationSchema.extend({
...
export async function suggestMedicationAction(
  ...
  const result = await suggestMedication(await createClient(), parsed.data)
  if (!result.ok) return { ok: false, error: result.error }
...
export async function approveSuggestionAction(
  ...
  const result = await approveSuggestion(await createClient(), parsed.data)
  if (!result.ok) return { ok: false, error: result.error }
...
export async function rejectSuggestionAction(
  ...
  const result = await rejectSuggestion(await createClient(), parsed.data)
  if (!result.ok) return { ok: false, error: result.error }
